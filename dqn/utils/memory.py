...
def sample_batch_indexes(low, high, size):
    ...
    if high - low >= size:
        ...
        warnings.warn(
            'Not enough entries to sample without replacement. Consider increasing your warm-up phase to avoid '
            'oversampling!')
...
class RingBuffer(object):
    ...
    def __getitem__(self, idx):
        """
        return element of the buffer at specific index

        # Argument
            idx(int) : index location

        # Return
            element of the buffer at a given location
        """
...
class Memory(object):
    def __init__(self, window_length, ignore_episode_boundaries=False):
        ...
        self.recent_terminal = deque(maxlen=window_length)

    def sample(self, batch_size, batch_idxs=None):
        raise NotImplementedError()

    def append(self, observation, action, rewars, terminal, training=True):
        self.recent_observations.append(observation)
        self.recent_terminal.append(terminal)

    def ger_recent_state(self, current_observation):
        ...
        for offset in range(0, self.window_length - 1):
            ...
            current_terminal = self.recent_terminal[current_idx - 1] if current_idx - 1 >= 0 else False
