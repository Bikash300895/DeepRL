    ...
    def reply(self, batch_size=32):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            target_f = self.model.predict(
                state)  # it will be the output of the model, but we know one (action, reward) pair of the output
            target_f[0][
                action] = target  # 0 for batch dim, action is the known action. update the return for that action, this is where the learning will happen
